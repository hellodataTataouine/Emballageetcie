                    ...
                    <div class="card tt-page-header">
                        <div class="card-body d-lg-flex align-items-center justify-content-lg-between">
                            <div class="tt-page-title">
                                <h2 class="h5 mb-lg-0">{{ localize('Unités') }}</h2>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="card mb-4" id="section-1">
                                ...
                                <div class="d-flex align-items-center justify-content-between px-4 pb-4">
                                    <span>{{ localize('Affichage') }}
                                        {{ $units->firstItem() }}-{{ $units->lastItem() }} {{ localize('de') }}
                                        {{ $units->total() }} {{ localize('résultats') }}</span>
                                    <nav>
                    ...
                    @can('add_units')
                        <form action="{{ route('admin.units.store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="card mb-4" id="section-2">
                                <div class="card-body">
                                    <h5 class="mb-4">{{ localize('Ajouter une nouvelle unité') }}</h5>

                                    <div class="mb-4">
                                        <label for="name" class="form-label">{{ localize('Nom de l\'unité') }}</label>
                                        <input type="text" name="name" id="name"
                                            placeholder="{{ localize('Saissisez Nom de l\'unité') }}" class="form-control" required>
                                    </div>
                                </div>
                            </div>
                            <!--brand info end-->
                                ...
                                <div class="col-12">
                                    <div class="mb-4">
                                        <button class="btn btn-primary" type="submit">
                                            <i data-feather="save" class="me-1"></i> {{ localize('Enregistrer l\'unité ') }}
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="tt-vertical-step">
                                <ul class="list-unstyled">
                                    <li>
                                        <a href="#section-1" class="active">{{ localize('Toutes les unités') }}</a>
                                    </li>
                                    @can('add_units')
                                        <li>
                                            <a href="#section-2">{{ localize('Ajouter une nouvelle unité') }}</a>
                                        </li>
                                    @endcan
                                </ul>
                            </div>
